...
{
    printf("MATRIX [%s] (%d x %d) is\n", name, row, column);
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < column; j++)
        {
            printf("%8.6f\t", A[j * row + i]);
        }
        printf("\n");
    }
    printf("\n");
...
void print_mat_complex(const char *name, int row, int column, complex *A)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < column; j++)
        {
            printf("(%8.6f, %8.6f)\t", A[j * row + i].real, A[j * row + i].imag);
        }
        printf("\n");
    }
    printf("\n");
...
#define printm(row, column, A) print_mat(#A, row, column, A)
